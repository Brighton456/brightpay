...
export async function finalizeDepositOrEndpoint(supabase: any, tx: any, opts: FinalizeOpts) {
...
export async function finalizeWithdrawal(supabase: any, tx: any, opts: FinalizeOpts) {
  const { data: current } = await supabase.from("transactions").select("status").eq("id", tx.id).single();
  if (current?.status === "completed") return;
